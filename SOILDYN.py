...
# !     Find and Read Soil Profile Section.
        ...
        SLNO,SLSOUR,SLTXS,SLDP,SLDESC = f60.read(CHARTEST)
...
#-----------------------------------------------------------------------
        ...
        SALB, U, SWCON, CN, DMOD, SLPF, SMPX = f80.read(CHARTEST)
...
#-----------------------------------------------------------------------
    ...
    # !       Set the soil pH to an average value, if not given.
            ...
            if ADCOEF[L] <= 0.0:
                ...
                if Length < 2:
                    ...
                    if MSG[3].find('ADCOEF') < 1:
                        MSG[3] = MSG[3].strip() + ', ADCOEF'
    ...
    #           Print message for missing or invalid data
                ...
                MSG[4]='These defaults may or may not be important to your results.'
              # CALL INFO(4, ERRKEY, MSG)

    #       Warning that the pH was set to a different value.
            if PHFLAG:
                MSG[1] = 'The pH of one or more soil layers at initialization'
                MSG[2] = 'was outside the range pH=1-10, which is unlikely.'
                MSG[3] = 'The pH of one or more soil layers at initialization'
...
# !-----------------------------------------------------------------------
    ...
    #         Use inorganic N values to calculate organic N in kg/ha
                NO3[L] = max (NO3[L], 0.01) #ppm
                NH4[L] = max (NH4[L], 0.01) #ppm
    #         TOTN in %
...
#-----------------------------------------------------------------------
#     Initialization
#  Designate the CN2, BD, and SWCN values from CROPGRO as the settled
#  values to be used in changing curve number, bulk density, and
#  hydraulic conductivity of the top 3 soil layers as affected by
#  tillage and rainfall kinetic energy
        BD_INIT   = BD
        CN_INIT   = CN
        DLAYR_INIT= DLAYR
        DS_INIT   = DS
...
# C       to rainfall kinetic energy.
            if (RAIN > 0.0 or TIL_IRR > 0.0) and TILLED:
                pass
              # CUMDEP  = 0.0
