...
def SUMVALS (NUMBER, LABEL, VALUE):
    #Updates values in SUMDAT
    ...
    for I in range(NUMBER):
      ...
      match label:
          # From OPHARV:
          case 'EDAT': sm.SUMDAT.EDAT = round(VALUE[I])
          case 'ADAT': sm.SUMDAT.ADAT = round(VALUE[I])
          case 'MDAT': sm.SUMDAT.MDAT = round(VALUE[I])
          case 'DWAP': sm.SUMDAT.DWAP = round(VALUE[I])
          case 'CWAM': sm.SUMDAT.CWAM = round(VALUE[I])
          case 'HWAM': sm.SUMDAT.HWAM = round(VALUE[I])
          case 'HWAH': sm.SUMDAT.HWAH = VALUE[I]  # Float
          case 'BWAH': sm.SUMDAT.BWAH = VALUE[I]  # Float
          case 'HWUM': sm.SUMDAT.HWUM = VALUE[I]  # Float
          case 'H#AM': sm.SUMDAT.HNUMAM = round(VALUE[I])
          case 'H#UM': sm.SUMDAT.HNUMUM = VALUE[I]  # Float
          case 'NFXM': sm.SUMDAT.NFXM = round(VALUE[I])
          case 'NUCM': sm.SUMDAT.NUCM = round(VALUE[I])
          case 'CNAM': sm.SUMDAT.CNAM = round(VALUE[I])
          case 'GNAM': sm.SUMDAT.GNAM = round(VALUE[I])
          case 'PWAM': sm.SUMDAT.PWAM = round(VALUE[I])
          case 'LAIX': sm.SUMDAT.LAIX = VALUE[I]  # Float
          case 'HIAM': sm.SUMDAT.HIAM = VALUE[I]  # Float

          # Fresh weight variables
          case 'FCWAM': sm.SUMDAT.FCWAM = round(VALUE[I])
          case 'FHWAM': sm.SUMDAT.FHWAM = round(VALUE[I])
          case 'HWAHF': sm.SUMDAT.HWAHF = VALUE[I]  # Float
          case 'FBWAH': sm.SUMDAT.FBWAH = VALUE[I]  # Float
          case 'FPWAM': sm.SUMDAT.FPWAM = round(VALUE[I])

          # From MgmtOps:
          case 'IR#M': sm.SUMDAT.IRNUM = round(VALUE[I])
          case 'IRCM': sm.SUMDAT.IRCM = round(VALUE[I])

          # From OPSPAM:
          case 'ETCM': sm.SUMDAT.ETCM = round(VALUE[I])
          case 'EPCM': sm.SUMDAT.EPCM = round(VALUE[I])
          case 'ESCM': sm.SUMDAT.ESCM = round(VALUE[I])

          # From OPWBAL:
          case 'PRCM': sm.SUMDAT.PRCM = round(VALUE[I])
          case 'ROCM': sm.SUMDAT.ROCM = round(VALUE[I])
          case 'DRCM': sm.SUMDAT.DRCM = round(VALUE[I])
          case 'SWXM': sm.SUMDAT.SWXM = round(VALUE[I])

          # From OpSoilNC:
          case 'NI#M': sm.SUMDAT.NINUMM = round(VALUE[I])
          case 'NICM': sm.SUMDAT.NICM = round(VALUE[I])
          case 'NLCM': sm.SUMDAT.NLCM = round(VALUE[I])
          case 'NIAM': sm.SUMDAT.NIAM = round(VALUE[I])
          case 'NMINC': sm.SUMDAT.NMINC = round(VALUE[I])
          case 'RECM': sm.SUMDAT.RECM = round(VALUE[I])

          case 'OCTAM': sm.SUMDAT.OCTAM = round(VALUE[I])
          case 'OCAM': sm.SUMDAT.OCAM = round(VALUE[I])
          case 'ONTAM': sm.SUMDAT.ONTAM = round(VALUE[I])
          case 'ONAM': sm.SUMDAT.ONAM = round(VALUE[I])
          case 'OPTAM': sm.SUMDAT.OPTAM = round(VALUE[I])
          case 'OPAM': sm.SUMDAT.OPAM = round(VALUE[I])

          # From OpSoilP
          case 'PI#M': sm.SUMDAT.PINUMM = round(VALUE[I])
          case 'PICM': sm.SUMDAT.PICM = round(VALUE[I])
          case 'PUPC': sm.SUMDAT.PUPC = round(VALUE[I])
          case 'SPAM': sm.SUMDAT.SPAM = round(VALUE[I])

          # From OpSoilK
          case 'KI#M': sm.SUMDAT.KINUMM = round(VALUE[I])
          case 'KICM': sm.SUMDAT.KICM = round(VALUE[I])
          case 'KUPC': sm.SUMDAT.KUPC = round(VALUE[I])
          case 'SKAM': sm.SUMDAT.SKAM = round(VALUE[I])

          # From OpStress
          case 'DMPPM': sm.SUMDAT.DMPPM = VALUE[I]
          case 'DMPEM': sm.SUMDAT.DMPEM = VALUE[I]
          case 'DMPTM': sm.SUMDAT.DMPTM = VALUE[I]
          case 'DMPIM': sm.SUMDAT.DMPIM = VALUE[I]
          case 'DPNAM': sm.SUMDAT.DPNAM = VALUE[I]
          case 'DPNUM': sm.SUMDAT.DPNUM = VALUE[I]

          case 'YPPM': sm.SUMDAT.YPPM = VALUE[I]
          case 'YPEM': sm.SUMDAT.YPEM = VALUE[I]
          case 'YPTM': sm.SUMDAT.YPTM = VALUE[I]
          case 'YPIM': sm.SUMDAT.YPIM = VALUE[I]
          case 'YPNAM': sm.SUMDAT.YPNAM = VALUE[I]
          case 'YPNUM': sm.SUMDAT.YPNUM = VALUE[I]

          case 'NDCH': sm.SUMDAT.NDCH = round(VALUE[I])
          case 'TMINA': sm.SUMDAT.TMINA = VALUE[I]
          case 'TMAXA': sm.SUMDAT.TMAXA = VALUE[I]
          case 'SRADA': sm.SUMDAT.SRADA = VALUE[I]
          case 'DAYLA': sm.SUMDAT.DAYLA = VALUE[I]
          case 'CO2A': sm.SUMDAT.CO2A = VALUE[I]
          case 'PRCP': sm.SUMDAT.PRCP = VALUE[I]
          case 'ETCP': sm.SUMDAT.ETCP = VALUE[I]
          case 'ESCP': sm.SUMDAT.ESCP = VALUE[I]
          case 'EPCP': sm.SUMDAT.EPCP = VALUE[I]

          # From GHG_Mod
          case 'N2OEM': sm.SUMDAT.N2OEM = VALUE[I]
          case 'CO2EM': sm.SUMDAT.CO2EM = VALUE[I]
          case 'CH4EM': sm.SUMDAT.CH4EM = VALUE[I]

          # From Ipexp or Ipwth:
          case 'YCRD': sm.SUMDAT.YCRD = VALUE[I]
          case 'XCRD': sm.SUMDAT.XCRD = VALUE[I]
          case 'ELEV': sm.SUMDAT.ELEV = VALUE[I]

          # Crop status
          case 'CRST': sm.SUMDAT.CRST = VALUE[I]

          # Economic Yield
          case 'EYLDH': sm.SUMDAT.EYLDH = VALUE[I]
...
# C=======================================================================
# def OPSUM (CONTROL, ISWITCH, YRPLT):
#
# C-----------------------------------------------------------------------
#       USE SumModule
#       USE ModuleDefs
