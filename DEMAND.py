...
def DEMAND(DYNAMIC, CONTROL,
        ...
        NDLEAF, NR1, NR2, NR5, NR7, NVEG0, YREND = [0]*7
        ...
        AGRSH2, AGRRT, AGRSTM, FRLFF, FRSTMF = [0.0]*5
        CARMIN, LIPOPT, LIPTB, SLOSUM = [0.0]*4

        AGRLF, AGRSD1, AGRSD2, AGRVG, AGRVG2 = [0.0]*5
        CDMREP, CDMSD, CDMSDR, CDMTOT = [0.0]*4
        CDMVEG, DRPP, DUMFAC, DXR57, F = [0.0]*5
        FNINL, FNINR, FNINS, FNINSD, FNINSH = [0.0]*5
        FRACDN, FRLF, FRLFMX = [0.0]*3
        FRRT, FRSTM, FVEG = [0.0]*3
        GDMSD, GDMSDR = [0.0]*2
        GROMAX, GRRAT1, LAGSD, LNGPEG, LNGSH = [0.0]*5
        NDMNEW, NDMOLD, NDMREP = [0.0]*3
        NDMSD, NDMSDR, NDMSH, NDMTOT, NDMVEG = [0.0]*5
        NMINEP, NMOBMX, NMOBR, NRCVR, NSTRES = [0.0]*5
        NVSMOB = 0.0
        PAR, PCNL, PCNRT, PCNST = [0.0]*4
        PGAVL, PLIGSD, PLTPOP, PMINSD, POASD = [0.0]*5
        PROLFF, PROLFI = [0.0]*2
        PRORTF, PRORTI, PROSTF, PROSTI, RCH2O = [0.0]*5
        RLIG, RLIP, RMIN, RNO3C = [0.0]*4
        ROA, RPRO, RPROAV, RTWT, SDGR = [0.0]*5
        SDLIP, SDPRO, SDVAR, SHLAG, SHVAR = [0.0]*5
        SIZELF, SIZREF, SLAMN, SLAMX, SLAPAR = [0.0]*5
        SRMAX, STMWT, SWFAC, TAVG, TDUMX = [0.0]*5
        SIZRAT, TDUMX2 = [0.0]*2
        TURADD, TURFAC, TURSLA, TURXFR = [0.0]*4
        VSSINK, VSTAGE, WCRLF, WCRRT, WCRST, WNRLF = [0.0]*6
        WNRRT, WNRSH, WNRST, WTLF, XFRMAX = [0.0]*5
        XFRT, XFRUIT, XPOD = [0.0]*3
        ...
        TGRO = np.zeros(TS)
        SDDES, SDNO, SHELN = (np.zeros(NCOHORTS) for _ in range(3))
        WTSD, WTSHE = (np.zeros(NCOHORTS) for _ in range(2))
        ...
        if DAS >= NR1:
                ...
                if DAS > NR2:
                        ...
                        SDCOMP(
                                ...
                                SDPRO, SLOSUM, TAVG, AGRSD1, AGRSD2,FNINSD, POTCAR, POTLIP
                        )
                ...
                TURXFR = min(max(XFRMAX * (1.0 - TURFAC), 0.0), 1.0)
...
# !=======================================================================
def IPDMND(
        FILECC, FILEGC, FILEIO,                         #Input e.g. \TMGRO048.SPE, \TMGRO048.ECO, \DSSAT48.INP
        CARMIN, FINREF, FNSDT, FRLFF, FRLFMX,           #Outut
        FRSTMF, LIPOPT, LIPTB, LNGSH, NMOBMX,           #Outut
        NRCVR, NVSMOB, PLIGSD, PMINSD, POASD,           #Outut
        PROLFF, PROLFI, PRORTF, PRORTI, PROSTF, PROSTI, #Outut
        RCH2O, RLIG, RLIP, RMIN, RNO3C, ROA,            #Outut
        RPRO, SDLIP, SDPRO, SHLAG, SLAMAX, SLAMIN,      #Outut
        SLAPAR, SLAREF, SLAVAR, SLOSUM, SIZELF, SIZREF, #Outut
        SRMAX, THRESH, TURSLA, TYPSDT, VSSINK, XFRMAX,  #Outut
        XFRUIT, XLEAF, XSLATM, XTRFAC, XVGROW, XXFTEM,  #Outut
        YLEAF, YSLATM, YSTEM, YTRFAC, YVREF, YXFTEM):   #Outut
        ERRKEY = 'DEMAND'
