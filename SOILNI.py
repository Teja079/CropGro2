#======================================================================
# SoilNi, Subroutine
## Determines inorganic N transformations
# This routine was modified from NTRANS when the module was split into
# organic and inorganic sections.
#----------------------------------------------------------------------
#-----------------------------------------------------------------------
#  Called : SOIL
...
def SoilNi (CONTROL, ISWITCH,CH4_data, DRN, ES, FERTDATA, FLOODWAT, IMM,LITC,
            MNR, newCO2, SNOW, SOILPROP, SSOMC, ST,SW, TDFC, TDLNO, TILLVALS,
            UNH4, UNO3, UPFLOW, WEATHER, XHLAI,FLOODN):
#
# !-----------------------------------------------------------------------
#       USE GHG_mod
#       USE FertType_mod
...
#       INTERFACE
#     def SoilNiBal(CONTROL, ISWITCH,ALGFIX, CIMMOBN, CMINERN, CUMFNRO, FERTDATA, NBUND, CLeach,CNTILEDR, TNH4, TNO3, TOTAML, TOTFLOODN, TUREA, WTNUP,N2O_data):
#           USE GHG_mod
#           USE FertType_mod
...
# !         Set initial SOM and nitrogen conditions for each soil layer.
        H4, NO3, SNH4, SNO3, TFNITY, UPPM, UREA = SoilNi_init (CONTROL, ISWNIT,SOILPROP, ST)
...
# !       Initialize flooded N if flooding is a possibility.
        ALGFIX, BD1, CUMFNRO, TOTAML, TOTFLOODN, DLTSNH4, DLTSNO3,
        DLTUREA, FLOODN, OXLAYR = (
        FLOOD_CHEM(CONTROL, ISWITCH,FLOODWat, LFD10, SOILPROP, SNO3,
        SNH4, UREA,SRAD, SW, TMAX, TMIN, XHLAI, DLTSNH4, DLTSNO3, DLTUREA, FLOODN, OXLAYR))
#
# !       Initialize TOTAML
        DLTSNH4, DLTSNO3, DLTUREA, OXLAYR, ALI, TOTAML = (
            OXLAYER(CONTROL, BD1, ES, FERTDATA, FLOODWAT, LFD10, NSWITCH,
                     SNH4, SNO3, SOILPROP, SRAD, ST,SW, TMAX, TMIN, UREA,
                    XHLAI,DLTSNH4, DLTSNO3, DLTUREA, OXLAYR))

        LFD10 = CONTROL.YRSIM
#
# !       Initialize denitrification routines
        if MEGHG == "1":
            CNOX, TNOXD, N2O_data, DLTSNO3 = Denit_DayCent (
                CONTROL, ISWNIT, dD0, newCO2, NO3, SNO3, SOILPROP)
        else:
            CNOX, TNOXD, N2O_data, DLTSNO3 = Denit_Ceres(
                CONTROL, ISWNIT, DUL, KG2PPM, LITC, NLAYR, NO3,
                SAT, SSOMC, SNO3, ST, SW, DLTSNO3)

#
# !     GHG emissions
        N2Oemit(CONTROL, ISWITCH, dD0, SOILPROP, N2O_data)
        OpN2O(CONTROL, ISWITCH, SOILPROP, N2O_data)
        OPGHG(CONTROL, ISWITCH, N2O_data, CH4_data)
#
        if CONTROL.RUN == 1 or not any(c in CONTROL.RNMODE for c in 'QF'):
            nox_pulse(dynamic, rain, snow, nox_puls)
...
# !     Check for fertilizer added today or active slow release fertilizers
        if FERTDAY == YRDOY or NActiveSR > 0:
            SUMFERT = 0.0
            for L in range(NLAYR):
                DLTSNO3[L] = DLTSNO3[L] + FERTDATA.ADDSNO3[L]
                DLTSNH4[L] = DLTSNH4[L] + FERTDATA.ADDSNH4[L]
                DLTUREA[L] = DLTUREA[L] + FERTDATA.ADDUREA[L]
                if FERTDATA.ADDUREA[L] > 1.0e-4:
                    IUON = True
                    IUYRDOY = INCDAT(YRDOY, 21)
                    YEAR, IUOF = YR_DOY(IUYRDOY)
                SUMFERT = SUMFERT + FERTDATA.ADDSNO3[L] + FERTDATA.ADDSNH4[L] + FERTDATA.ADDUREA[L]
...
# !       tillage depth
        if TILLVALS.NTIL > 0:
            ...
            if YRDOY == TILDATE:
                ...
                SoilMix(SNO3, DLTSNO3, 1, DLAYR, MIXPCT, NLAYR, TDEP)
                SoilMix(SNH4, DLTSNH4, 1, DLAYR, MIXPCT, NLAYR, TDEP)
                SoilMix(UREA, DLTUREA, 1, DLAYR, MIXPCT, NLAYR, TDEP)
...
# !         with top soil layer.
        if NBUND > 0:
            (DLTSNH4, DLTSNO3, DLTUREA, FLOODN, OXLAYR, ALGFIX, BD1, CUMFNRO,
             TOTAML, TOTFLOODN) = FLOOD_CHEM( CONTROL, ISWITCH,FLOODWat, LFD10,
                                              SOILPROP, SNO3, SNH4, UREA,SRAD, SW,
                                              TMAX, TMIN, XHLAI,DLTSNH4, DLTSNO3, DLTUREA, FLOODN, OXLAYR)                      # Input
#
# !     This needs to be called from NTRANS for upland cases, too.
        if FLOOD <= 0.0:
            DLTSNH4, DLTSNO3, DLTUREA, OXLAYR, ALI, TOTAML = OXLAYER(
                CONTROL, BD1, ES, FERTDATA, FLOODWAT, LFD10, NSWITCH,
                SNH4, SNO3, SOILPROP, SRAD, ST,SW, TMAX, TMIN, UREA,
                XHLAI,DLTSNH4, DLTSNO3, DLTUREA, OXLAYR)
#
# !     ----------------------------------------------------------------
...
# !     Diffusivity rate calculation from DayCent
        DayCent_diffusivity(dD0, sw, soilprop)
# !-----------------------------------------------------------------------
#
# !     ------------------------------------------------------------------
...
# !-----------------------------------------------------------------------
            if IUON:
                ...
                if (YRDOY < UIData.UIEND) and (L <= UIData.UILYR):
                    AK = AK * (1.0 - UIData.UIEFF / 100.)

                # Calculate the soil water factor for the urea hydrolysis, and limit it between 0 and 1.
                if FLOOD > 0.0:
                    WFUREA = 1.0
                else:
                    WFUREA = WFSOM + 0.20
                    WFUREA = max(min(WFUREA, 1.0), 0.0)

                # Calculate the amount of urea that hydrolyses.
                UHYDR = AK * min(WFUREA, TFUREA) * (UREA[L] + DLTUREA[L])
                UHYDR = min(UHYDR, UREA[L] + DLTUREA[L])
...
# !           add the mineralized N to the NH4 pool.
            if NNOM >= 0.0:
                ...
                if abs(NNOM) > (SNH4[L] + DLTSNH4[L] - XMIN):
                    ...
                    if abs(NNOM) > (SNO3_AVAIL + DLTSNO3[L] - XMIN):
                    ...
                    # Reduce soil NH4 by the immobilization (NNOM is negative!)
                    DLTSNH4[L] = DLTSNH4[L] + NNOM
                    # TIMMOBILIZE[N] = TIMMOBILIZE[N] - NNOM
                    NNOM = 0.0
...
# Nitrification based on Gilmour
            TKELVIN = ST[L] + 273.0
            TFACTOR = math.exp(-6572 / TKELVIN + 21.4)  # 6602
...
#
            PHFACT  = min(1.0, 0.33 * PH[L] - 1.36)
# !        NH4(L)  = SNH4(L)*KG2PPM(L)
            if (FLOOD > 0.0):
                PHFACT = 1.0
                WF2    = 0.0
                TFNITY(L) = 0.0
            T2   = max (0.0,(TFNITY(L) - 1.0))
            TLAG = 0.075 * T2**2
            TLAG = min(TLAG,1.0)
            if (NBUND <= 0.):
              TLAG = 1.0
            NFAC = max(0.0, min(1.0, TFACTOR * WF2 * PHFACT * TLAG))
# !       If nitrification inhibitor is active, reduce rate
            if ((YRDOY < NIData.NIEND) and (L <= NIData.NILYR)):
...
#         ENDIF
            XMIN = 0.0
            SNH4_AVAIL = max(0.0, SNH4[L] + DLTSNH4[L] - XMIN)
            NITRIF[L] = min(NITRIF[L], SNH4_AVAIL)
...
# !       ------------------------------------------------------------------
            if NITRIF[L] > 1.0e-6:
                ...
                if potential_NOflux <= NITRIF_remaining:
                    ...
                    NH4_to_NO = 0.0
                    # NH4_to_NO = min(SNH4_AVAIL, (potential_NOflux - NITRIF_remaining))
                    NITRIF_remaining = 0.0

                nNOflux[L] = max(NITRIF_to_NO + NH4_to_NO, 0.0)
...
#       !if (sum(n2odenit) > 1.e-9) then
        for L in range(NLAYR):
            potential_NOflux = NO_N2O_ratio[L] * N2ODenit[L] * min(1.0, krainNO)
            SNH4_AVAIL = max(0.0, SNH4[L] + DLTSNH4[L] - XMIN)

            if potential_NOflux <= SNH4_AVAIL:
                # Take all N out of ammonium pool
                ...
                # if potential_NOflux <= N2ODenit[L]:
...
#
        if IUON:
            ...
            NFLUX(
                ...
                DLTUREA, CLeach, TLeachD, CNTILEDR, NTILEDR    # Output
            )
        ...
        NFLUX(
            ...
            DLTSNO3, CLeach, TLeachD, CNTILEDR, NTILEDR        # Output
        )

        PUT("NITR", "TLCHD", TLeachD)

        # Pseudo-integration - for plant-available N
        for L in range(NLAYR):
