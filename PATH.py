...
#=======================================================================
#  PATH, Subroutine
#  Program to read the PATH from DSSATPRO file
#-----------------------------------------------------------------------
#  INPUT  : PROCOD - 3 letter folder acronim in DSSATPRO,
#           DSSATP - full path to DSSATPRO,
#           PFLAG - integer (1, ?)
#  OUTPUT : PATHC - folder name with trailing slash symbol
#           NAMEF - 13 symbol after folder name or spaces
#=======================================================================
def PATH(PROCOD,DSSATP,PFLAG):
    ...
    FEXIST = INQUIRE(DSSATP)
    if not FEXIST:
        return PATHC, NAMEF
    else:
        with open(DSSATP) as f:
            for LINE in f:
                I = I + 1
                if LINE[0:3] == PROCOD:
                    PATHC = LINE.split(' ')[1] + LINE.split(' ')[2]

                    # PATHL  = PATHC.find(BLANK)   VSH
                    PATHL = len(PATHC)
                    if PATHL == 0:
                        ERROR (ERRKEY,3,DSSATP,I)
                    if PATHC[PATHL-1:PATHL] != '\\':
                        PATHC = PATHC.strip() + '\\'
                    else:
                        PATHL = PATHL - 1

                    NAMEF  = PATHC[PATHL+1:PATHL+14]
                    if PATHL < 80:
                        PATHC_list = list(PATHC)
                        for K  in range(PATHL+1,80):
                            if PATHC_list[K-1:K] != BLANK:
                                PATHC_list[K-1:K] = BLANK
                        PATHCS = "".join(PATHC_list)
                    return PATHCS, NAMEF

#=======================================================================
#  PATHD, Subroutine
#
#  Program to read the PATH of DSSATPRO file
#    Assume in current directory first.
#    If not in current directory, DSSATPRO file is stored with the EXEs
#-----------------------------------------------------------------------
#  Revision history
#
#  1. Written                                       G.H         8-10-93
#  2  Modified by
#  3. Header revision and minor changes             P.W.W.      5-28-93
# 07/17/2008 CHP DSSATPRO name specified in ModuleDefs
#-----------------------------------------------------------------------
#  INPUT  : INPUTX
#
#  LOCAL  : BLANK ERRKEY FILEPR LINE LUNPR ERRNUM PATHL I K FEXIST
#
#  OUTPUT : DSSATP
#-----------------------------------------------------------------------
#  Called : INPUT
...
# Test driver for PATH
# PROCOD = 'WED'
# DSSATP = 'C:/DSSAT48/DSSATPRO.v48'
# PFLAG = 1
#
# PATHC, NAMEF = PATH(PROCOD,DSSATP,PFLAG)
#
# print(PATHC)
# print(NAMEF)