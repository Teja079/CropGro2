...
# Called by: WEATHR
#======================================================================
def CO2VAL(CONTROL, ISWITCH, CCO2, DCO2):
# !-----------------------------------------------------------------------
#       USE ModuleDefs
...
#
#       INTEGER DOY, DYNAMIC, ERR, FOUND, I,
    INDX : int = 1
#       INTEGER LINE, LNUM, LUNCO2, PFLAG, YEAR
    NVals : int = 0
#       LOGICAL FEXIST, LEAP
#
#       CHARACTER*6, PARAMETER :: ERRKEY = 'CO2VAL'
    MaxNVals : int  = 5000  #Max number of records
    FYEAR = np.zeros(MaxNVals, dtype=np.int_)
    FDOY = np.zeros(MaxNVals, dtype=np.int_)
    FCO2 = np.zeros(MaxNVals, dtype=float)
    CO2BAS : float = 0.0
#
